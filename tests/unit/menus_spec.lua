...
describe('wave-1 menus', function()
    ...
    describe('about', function()
        it('shows the server info entry only when configured', function()
            ...
            -- version + credits + Lua-rewrite credit
            assert.equal(3, plain.menu:Size())
            ...
            assert.equal(4, with_info.menu:Size())
    ...
    describe('vehicle spawner', function()
        ...
        it('hides the replace previous toggle without its permission', function()
            grant({ VSMenu = true })
            allow_all_categories()
            local VehicleSpawner = require('client.menus.vehicle_spawner')
            local instance = VehicleSpawner.create()
            assert.is_nil(find_item(instance.menu, 'Replace Previous Vehicle'))
            assert.is_not_nil(find_item(instance.menu, 'Spawn Inside Vehicle'))
        end)
