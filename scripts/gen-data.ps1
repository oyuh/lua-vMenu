...
# --- ped_appearance_models.lua ------------------------------------------------------
# PlayerAppearance.cs embeds its spawnable-ped dictionaries in the menu class
# instead of vMenu/data; extracted here so the Lua menu stays readable.

$lines = Get-Content (Join-Path $UpstreamPath 'vMenu\menus\PlayerAppearance.cs')
$sb = [System.Text.StringBuilder]::new()
[void]$sb.AppendLine("-- GENERATED by scripts/gen-data.ps1 from vMenu/menus/PlayerAppearance.cs @ $pin — DO NOT EDIT.")
[void]$sb.AppendLine('-- luacheck: ignore')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('local M = {}')
[void]$sb.AppendLine('')
Emit-OrderedDict $sb 'main_models' (Parse-IndexerDict (Get-Region $lines 'Dictionary<string, string>\s+mainModels\b'))
Emit-OrderedDict $sb 'animal_models' (Parse-IndexerDict (Get-Region $lines 'Dictionary<string, string>\s+animalModels\b'))
Emit-OrderedDict $sb 'male_models' (Parse-IndexerDict (Get-Region $lines 'Dictionary<string, string>\s+maleModels\b'))
Emit-OrderedDict $sb 'female_models' (Parse-IndexerDict (Get-Region $lines 'Dictionary<string, string>\s+femaleModels\b'))
Emit-OrderedDict $sb 'other_peds' (Parse-IndexerDict (Get-Region $lines 'Dictionary<string, string>\s+otherPeds\b'))
Save-Output $sb 'ped_appearance_models.lua'
