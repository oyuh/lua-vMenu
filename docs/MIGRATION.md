...
The whole migration is: stop the server, swap the resource folder, start the server. Nothing
else changes. Not your config, not your permissions, not your players' saves.
...
3. **Copy your config across.** Take these from your old vMenu folder and put them in this one:
   ...
   Any of these you never customized can be skipped, since defaults ship with this resource.
4. **Replace the folder.** Delete the old `resources/vMenu` and put this resource in its place,
   keeping the folder name `vMenu`. This part actually matters: client save data (KVP) is keyed
   by resource name, so a different folder name would orphan every player's saved
   vehicles/peds/loadouts.
5. **Start the server.** Your existing `permissions.cfg` is read unchanged: every `vmenu_*`
   convar and `vMenu.*` ace permission, including `vMenu.Staff.All` style groups and the
   supplementary `vMenu.VehicleWhitelist.*` / `PedWhitelist.*` / `WeaponWhitelist.*` aces.
...
- A handful of upstream bugs where the code plainly contradicts its intent were fixed, and each
  fix is marked with a comment in the source (e.g. the Ped Collections menu that could never
  populate). Load-bearing quirks (serialization typos, swapped assignments that affect saved
  data) are all kept as-is.
- **Menu input is arrow-keys-only on keyboard.** Upstream (via MenuAPI) also lets WASD drive
  the menu, because the game binds its nav controls to WASD as well as the arrow keys. This
  resource reads the physical arrow keys directly, so WASD stays free for movement. The full
  keyboard/mouse scheme: arrows to move, Enter to select, Backspace/Escape to go back,
  left-click to select, right-click to go back, scroll wheel to move up/down. Controller
...
*empty* lists (say, an MP character with no tattoos saved by this resource) may serialize as
`{}` where Newtonsoft expects `[]`. Characters saved by C# vMenu are untouched unless you
re-save them here. If you plan to A/B test, back up player KVPs first.

## Troubleshooting

- **Menu doesn't open:** check `vmenu_use_permissions`. With permissions enabled a player needs
  at least one `vMenu.*` ace, and staff-only mode (`vmenu_menu_staff_only`) requires
  `vMenu.Staff`.
- **Players lost their saves:** the resource folder isn't named `vMenu`. Rename it back and
  they'll come back. The KVPs are still on the clients, keyed to the old name.
